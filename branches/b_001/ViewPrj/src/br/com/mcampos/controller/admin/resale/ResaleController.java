...
public class ResaleController extends BasicListController<ResaleDTO>
{
    ...
    protected Object saveRecord( Object currentRecord )
    {
        ...
        if ( client == null )
            throw new InvalidParameterException( "Client inválido. Selecione um cliente" );
    ...
    protected void persist( Object e ) throws ApplicationException
    {
        if ( isAddNewOperation() )
            getSession().add( getLoggedInUser(), ( ResaleDTO )e );
        else
            getSession().update( getLoggedInUser(), ( ResaleDTO )e );
    ...
    {
        if ( session == null )
            session = ( ResaleFacade )getRemoteSession( ResaleFacade.class );
        return session;
    }
    ...
    {
        if ( listClientCompany.getSelectedItem() == null )
            return null;
