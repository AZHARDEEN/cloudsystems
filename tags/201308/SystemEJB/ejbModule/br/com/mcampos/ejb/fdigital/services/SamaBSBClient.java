...
public class SamaBSBClient
{
	...
	private List<PgcPageDTO> getPgcs( Connection conn ) throws SQLException
	{
		...
		if ( list.size( ) > 0 ) {
			if ( list.size( ) == 1 ) {
				logger.info( "Found just " + list.size( ) + " record to export to SAMA" );
			}
			else {
	...
	private void updateRecordStatus( Connection conn, PgcPageDTO page ) throws SQLException
	{
		String sql;

		sql = "UPDATE pgc_page SET ppg_exported_bt = TRUE where not ppg_exported_bt AND pgc_id_in = " + page.getPgc_id_in( );
		logger.error( "Updating PGC_PAGE where pgc_id_in =  " + page.getPgc_id_in( ) );
