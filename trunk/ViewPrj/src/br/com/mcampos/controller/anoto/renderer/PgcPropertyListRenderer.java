...
public class PgcPropertyListRenderer implements ListitemRenderer
{
	...
	private void insertDescription( Listitem item, PgcPropertyDTO dto )
	{
		switch ( sequence ) {
			case 3: item.getChildren().add( new Listcell( "Latitude" ) );
				item.getChildren().add( new Listcell( dto.getValue() ) );
				break;
			case 4: item.getChildren().add( new Listcell( "Longitude" ) );
				item.getChildren().add( new Listcell( dto.getValue() ) );
				break;
			case 5: item.getChildren().add( new Listcell( "Altitude" ) );
				item.getChildren().add( new Listcell( getHexValue( dto.getValue() ) + " M" ) );
				break;
			case 6: item.getChildren().add( new Listcell( "Precisão" ) );
				item.getChildren().add( new Listcell( getHexValue( dto.getValue() ) + " M" ) );
				break;
			default: item.getChildren().add( new Listcell( "N/A" ) );
