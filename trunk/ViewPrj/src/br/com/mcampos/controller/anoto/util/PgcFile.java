...
public class PgcFile {
    ...
    public PGCDTO persist(ArrayList<MediaDTO> medias) throws ApplicationException {
        ...
        PGCDTO insertedPgc =
            getSession().add(getCurrentPgc(), addresses, medias, properties);

        setCurrentPgc(insertedPgc);
        if (insertedPgc.getPgcStatus().getId() != PgcStatusDTO.statusOk)
            return insertedPgc;
        List<PgcPenPageDTO> pgcsPenPage =
            getSession().getPgcPenPages(insertedPgc);
        ...
        setCurrentPen(PadFile.getPen(pgc.getMedia().getObject(),
                                     form.getApplication()));
        try {
    ...
    private void processPage(PgcPenPageDTO pgcPenPage, int nBookIndex,
                             int nPageIndex, Page page) {
        String basePath;
        ...
        try {
            PgcPageDTO dto =
                new PgcPageDTO(pgcPenPage.getPgc(), nBookIndex, nPageIndex);
            dto.setAnotoPage(new AnotoPageDTO(pgcPenPage.getPenPage().getPage().getPad(),
                                              page.getPageAddress()));
            getSession().add(dto);
            List<MediaDTO> backImages =
                addAnotoImages(pgcPenPage, page, basePath, nBookIndex,
                               nPageIndex);
            if (SysUtils.isEmpty(pgcPenPage.getPenPage().getPage().getIcrTemplate()) ==
                false) {
                File icrTemplateFile =
                    new File(pgcPenPage.getPenPage().getPage().getIcrTemplate());
    ...
    private List<MediaDTO> addAnotoImages(PgcPenPageDTO pgcPenPage, Page page,
                                          String basePath, int nBookIndex,
                                          int nPageIndex) throws RenderException,
                                                                 NotAllowedException,
                                                                 ApplicationException {
        Renderer renderer;
        MediaDTO media;
        List<MediaDTO> backgroundImages =
            loadBackgroundImages(pgcPenPage, page);
        String renderedImage =
            basePath + "/renderedImage." + getImageFileTypeExtension();
        ...
        if (SysUtils.isEmpty(backgroundImages)) {
            ...
            if (media != null)
                getSession().addProcessedImage(pgcPenPage.getPgc(), media,
                                               nBookIndex, nPageIndex);
        } else {
            for (MediaDTO image : backgroundImages) {
                String imagePath = basePath + "/background";
                byte[] object = getSession().getObject(image);
                renderer.setBackground(getPad().saveBackgroundImage(imagePath,
                                                                    image.getName(),
                                                                    object));
                renderer.renderToFile(renderedImage);
                media = createMedia(renderedImage);
    ...
    private Map<String, IcrField> processIcr(PgcPenPageDTO pgcPenPage,
                                             Page page,
                                             String basePath) throws RenderException,
                                                                     NotAllowedException,
                                                                     ApplicationException {
        Renderer renderer;
        ...
        renderer = RendererFactory.create(page);
        icrImage =
                String.format("%s/%s_%s.%s", basePath, pgcPenPage.getPageAddress(),
                              "icr", "JPG");
        renderer.renderToFile(icrImage, 300);
        icrFields =
                ICRObject.processImage(pgcPenPage.getPenPage().getPage().getIcrTemplate(),
                                       icrImage, A2iaDocument.typeJPEG);
    ...
    private void addAttachments(PgcPageDTO pgcPage,
                                Page page) throws PageException,
                                                  ApplicationException {
        ...
        if (page.hasAttachments()) {
            ...
            while (it != null && it.hasNext()) {
                ...
                System.out.println("O pgc recebido possui anexos. Tipo " +
                                   obj.getType());
                if (obj.getType() == Attachment.ATTACHMENT_TYPE_BARCODE) {
                    ...
                    String sValue = "";
                    Byte caracter;
                    for (int nCount = 1; nCount < barCodeData.length;
                         nCount++) {
    ...
    private void addFields(PgcPageDTO pgcPage, Page page, String basePath,
                           Map<String, IcrField> icrFields) throws ApplicationException,
                                                                   PageAreaException {
        ...
        while (it != null && it.hasNext()) {
            ...
            if (icrFields != null) {
                ...
                if (field != null && field.getValue() != null) {
                    ...
                    icrValue = icrValue.replaceAll("_", " ").trim();
                    fieldDTO.setIrcText(icrValue);
                }
            }
    ...
    private void getFieldImage(String basePath, PageArea pageArea,
                               PgcFieldDTO fieldDTO) throws RenderException,
                                                            NotAllowedException {
        Renderer renderer;
        String path;
    ...
    private List<MediaDTO> loadBackgroundImages(PgcPenPageDTO pgcPenPage,
                                                Page page) {
        ...
        try {
            AnotoPageDTO dto =
                new AnotoPageDTO(pgcPenPage.getPenPage().getPage().getPad(),
                                 page.getPageAddress());
    ...
    public void setObject(PadFile pad,
                          byte[] object) throws PenCreationException {
        setPad(pad);
        bytePgc = object;
