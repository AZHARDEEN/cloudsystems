DROP VIEW inep.revisorextractview;
 ...
 WITH task_grade AS (
         SELECT s.usr_id_in, 
            ...
            ( SELECT d.dis_grade_in AS grade2
             ...
             WHERE d.usr_id_in = s.usr_id_in AND d.pct_id_in = s.pct_id_in AND d.isc_id_ch::text = s.isc_id_ch::text AND d.tsk_id_in = s.tsk_id_in AND d.col_seq_in <> s.col_seq_in AND d.dis_grade_in IS NOT NULL AND r.rvs_coordinator_bt = false AND d.dis_grade_in IS NOT NULL AND coalesce ( d.dis_golden_bt, false ) is false
            ...
            ( SELECT d.dis_grade_in AS grade3
                   FROM inep.inep_distribution d
              JOIN inep.inep_revisor r ON r.usr_id_in = d.usr_id_in AND r.pct_id_in = d.pct_id_in AND r.tsk_id_in = d.tsk_id_in AND r.col_seq_in = d.col_seq_in
             WHERE d.usr_id_in = s.usr_id_in AND d.pct_id_in = s.pct_id_in AND d.isc_id_ch::text = s.isc_id_ch::text AND d.col_seq_in <> s.col_seq_in AND d.tsk_id_in = s.tsk_id_in AND d.dis_grade_in IS NOT NULL AND r.rvs_coordinator_bt = true AND d.dis_grade_in IS NOT NULL AND coalesce ( d.dis_golden_bt, false ) is false
             ORDER BY d.usr_id_in, d.pct_id_in, d.isc_id_ch, d.tsk_id_in, d.col_seq_in
            OFFSET 0
            LIMIT 1) AS grade3
           FROM inep.inep_distribution s
          WHERE s.dis_grade_in IS NOT NULL
          AND coalesce ( s.dis_golden_bt, false ) is false
...
DROP VIEW inep.gradeview2;
DROP VIEW inep.gradeview;
DROP VIEW inep.distribution_view;
...
CREATE OR REPLACE VIEW inep.distribution_view AS 
 SELECT users.usr_name_ch, 
  ...
  and coalesce ( inep_distribution.dis_golden_bt, false ) is false;
 ...
 WITH task_grade AS (
         SELECT DISTINCT s.usr_id_in, 
            ...
            ( SELECT d.dis_grade_in AS task1_grade_1
             ...
             and coalesce ( d.dis_golden_bt, false ) is false
             ORDER BY d.usr_id_in, d.pct_id_in, d.isc_id_ch, d.tsk_id_in, d.col_seq_in
            OFFSET 0
